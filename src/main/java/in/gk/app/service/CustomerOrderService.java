...
public class CustomerOrderService {
	...
	public List<Object[]> getLineItemsAndAggrigatedQuantity() {
		EntityManager session = entityManagerFactory.createEntityManager();

		/*
		 * String query =
		 * "Select cu.Name, cuo.status, cuo.paid, cuo.orderTime, cuo.price, ol.quantity, pr.name From CustomerOrder AS cuo INNER JOIN Customer as cu ON cu.Customer_id = cuo.Customer_id INNER JOIN OrderLineItem as ol ON ol.order_id = cuo.Order_id INNER JOIN Product as pr ON pr.Products_id = ol.Product_id"
		 * ; String query1 =
		 * "Select cu.name as cusname, cuo.status, cuo.paid, cuo.order_time, cuo.price, ol.quantity, pr.name as prodname From customer_order AS cuo INNER JOIN Customer as cu ON cu.id = cuo.customer_id INNER JOIN order_line_item as ol ON ol.orderid = cuo.id INNER JOIN Product as pr ON pr.id = ol.productid"
		 * ; String query3 =
		 * "Select cu.name as cusname, cuo.status, cuo.paid, cuo.ordertime, cuo.price, ol.quantity, pr.name as prodname From customerorder AS cuo INNER JOIN customer as cu ON cu.id = cuo.customerid INNER JOIN orderlineitem as ol ON ol.orderid = cuo.id INNER JOIN product as pr ON pr.id = ol.productid"
		 * ;
		 * 
		 * String query2 =
		 * "Select cu.id as cid, cu.name as cname, cu.email as cemail,cu.phone as cphone from customer AS cu"
		 ...
		 */ String query5 = "Select cu.id as cid, cu.name as cname, cu.email as cemail,cu.phone as cphone, "
				...
				+ "ol.quantity as olquantity, " + "pr.id as pid, pr.name as pname, pr.price as pprice "
				...
				+ " INNER JOIN product as pr ON pr.id = ol.productid " + "	Where cuo.status = 'false'";
		String query6 = "Select pr.id, pr.name, pr.price, sum(ol.quantity) as quantity " + "FROM customerorder AS cuo"
				+ " INNER JOIN orderlineitem as ol ON ol.orderid = cuo.id "
				+ " INNER JOIN product as pr ON pr.id = ol.productid " + "	Where cuo.status = 'false' "
		...
		/*
		 * System.out.println(results.get(0)[0]); System.out.println(results.get(0)[1]);
		 */
	...
	public List<Object[]> getOrderCustomer() {
		EntityManager session = entityManagerFactory.createEntityManager();
		...
		String query7 = "Select cu.id as cid, cu.name as cname, cu.email as cemail,cu.phone as cphone, "
				+ "cuo.id as coid, cuo.customerid as cocustomerid, cuo.price as coprice, cuo.paid as copaid, "
				+ "cuo.ordertime as coordertime, cuo.orderfinishtime as coorderfinishtime, cuo.status as costatus"
				+ " FROM customerorder AS cuo INNER JOIN customer as cu ON cu.id = cuo.customerid"
		...
		@SuppressWarnings("unchecked")
		List<Object[]> results = session.createNativeQuery(query7, "orderCustomerMapping").getResultList();
		/*
		 * System.out.println(results.get(0)[0]); System.out.println(results.get(0)[1]);
		 */
